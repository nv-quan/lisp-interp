...
//state type: pos, row, col. This function initilize a new mpc_state_t var
...
typedef struct {
  char mem[64];
} mpc_mem_t; //arr of char with 64 byte
...
typedef struct {
  ...
  char *string;
  char *buffer; //what does buffer do?
  FILE *file;
  ...
  size_t mem_index; //??
  char mem_full[MPC_INPUT_MEM_NUM]; //create an array of char with 512 members -> flag?
  mpc_mem_t mem[MPC_INPUT_MEM_NUM]; //create an array of mpc_mem_t (this is an array it self) with 512 members
  
} mpc_input_t;

static mpc_input_t *mpc_input_new_string(const char *filename, const char *string) {

  mpc_input_t *i = malloc(sizeof(mpc_input_t)); //create memmory
  
  i->filename = malloc(strlen(filename) + 1); //+1 because of the terminating character
  strcpy(i->filename, filename);
  i->type = MPC_INPUT_STRING; //type: input
  
  i->state = mpc_state_new(); //initilize a state
  ...
  i->backtrack = 1; //?
  i->marks_num = 0; //?
  i->marks_slots = MPC_INPUT_MARKS_MIN; //=32
  i->marks = malloc(sizeof(mpc_state_t) * i->marks_slots); //-> create slots
  i->lasts = malloc(sizeof(char) * i->marks_slots); //arr of char
  i->last = '\0';
  
  i->mem_index = 0;
  memset(i->mem_full, 0, sizeof(char) * MPC_INPUT_MEM_NUM); // set all elements of mem_full to 0
  
  return i; // return mpc_input_t
}
...
static void *mpc_malloc(mpc_input_t *i, size_t n) {
  ...
  if (n > sizeof(mpc_mem_t)) { return malloc(n); } //return malloc(n) if n > size of mpc_mem_t (64 byte char)
  //else
  j = i->mem_index;
  do {
    /*find an empty member of "mem" and return that member to p*/
    if (!i->mem_full[i->mem_index]) { // if mem_full[mem_index] == 0
      p = (void*)(i->mem + i->mem_index); // p = the "index"th member of mem (arr of 64 char)
      i->mem_full[i->mem_index] = 1; // assign the "index"th member of mem_full to 1
      i->mem_index = (i->mem_index+1) % MPC_INPUT_MEM_NUM; //increase mem_index without exceeding the limit
      return p; 
    }
    //else
    i->mem_index = (i->mem_index+1) % MPC_INPUT_MEM_NUM; 
  } while (j != i->mem_index);
  //if all mem is full, return malloc(n)
  return malloc(n);
}
...
static mpc_err_t *mpc_err_fail(mpc_input_t *i, const char *failure) {
  mpc_err_t *x;
  if (i->suppress) { return NULL; } //return null if suppress == 1
  x = mpc_malloc(i, sizeof(mpc_err_t)); //create memmory for x, change state of mem_full in mpc_input_t
  x->filename = mpc_malloc(i, strlen(i->filename) + 1);
  strcpy(x->filename, i->filename);
  x->state = i->state;
  x->expected_num = 0;
...
int mpc_parse(const char *filename, const char *string, mpc_parser_t *p, mpc_result_t *r) {
  int x;
  mpc_input_t *i = mpc_input_new_string(filename, string); //create and copy to a new mpc_input_t
